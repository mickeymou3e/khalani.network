...
for i in {1..26}; do
    ...
    if [ ! -f "./artifact/srs/${new_filename}" ]; then
        ...
        wget -O "./artifact/srs/${new_filename}" "$url"
    else
        echo "File already exists at: /artifact/srs/${new_filename}. Skipping download."
    fi
done