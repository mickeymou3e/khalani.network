...
from uniswap_v3.uniswap_position_utils import uniswap_v3_position_state, UniswapV3PositionState
...
with open(POSITION_MANAGER_ABI_PATH, "r") as file:
    contract_data = json.load(file)
    POSITION_ABI = contract_data["abi"]

with open(FACTORY_ABI_PATH, "r") as file:
    contract_data = json.load(file)
    FACTORY_ABI = contract_data["abi"]
...
# ):
#     position_state: UniswapV3PositionState = uniswap_v3_position_state(
#         position_id, client
#     )

#     pool_address = position_state.pool_address
