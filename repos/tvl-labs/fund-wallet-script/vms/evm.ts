...
export default class EVM {
  ...
  shouldRecalibratePeriodically: boolean;
  isRecalibrating: boolean;
  lastRecalibrationTimestamp: number;

  intervalIdRecalibration?: NodeJS.Timeout;
  intervalIdRequestsProcessing?: NodeJS.Timeout;
  ...
  constructor(
    ...
    this.config = config;
    this.shouldRecalibratePeriodically = shouldRecalibratePeriodically;
  ...
  async start() {
    ...
    if (this.shouldRecalibratePeriodically) {
      this.intervalIdRecalibration = setInterval(() => this.recalibrate(), 1000);
    }

    this.intervalIdRequestsProcessing = setInterval(() => this.scheduleFromMemPool(), 1000)
  }

  async stop() {
    if (this.intervalIdRequestsProcessing) {
      clearInterval(this.intervalIdRequestsProcessing)
    }
    if (this.intervalIdRecalibration) {
      clearInterval(this.intervalIdRecalibration)
    }
  }
  ...
  async sendToken(
    ...
    let amount: BN = calculateBaseUnit(
      this.config.DRIP_AMOUNT,
    ...
    return new Promise((resolve) => {
      const statusCheckerInterval = setInterval(async () => {
        ...
        if (
          requestStatus.type === "mem-pool" ||
          requestStatus.type === "queueing"
        ) {
        ...
        if (requestStatus.type === "sent") {
          resolve({
            status: 200,
            message: `Transaction sent on ${this.config.NAME}!`,
            txHash: requestStatus.txHash,
          });
          this.log.info("Respond HTTP 200", {
            request,
            txHash: requestStatus.txHash,
          });
          return;
  ...
  private async updateBalances(): Promise<void> {
    try {
      ...
      this.log.info(
        ...
        {
          native_balance: calculatePresentableUnit(
            this.balance,
      ...
      this.log.error(
        `Recalibration failed for chain ${this.config.NAME} native balance: ${e.message}`
      );
    }
    ...
    for (const [_, contract] of Array.from(this.contracts.entries())) {
      try {
        ...
        this.log.error(
          `Recalibration failed for chain ${this.config.NAME} token ${contract.id} (${contract.address}): ${e.message}`
        );
      }
    }
  }
  ...
  private async processRequest(request: RequestType) {
    ...
    if (!this.isFaucetBalanceEnough(request)) {
      this.log.error(
        `Faucet balance is too low! ${request.id}: ${this.getBalance(
          request.id
        )}`
      );
  ...
  private scheduleFromMemPool() {
    const memPoolEntry = Array.from(this.requestStatus.entries()).find(
      ([, request]) => request.type === "mem-pool"
    );
    if (!memPoolEntry) {
      return;
    }
    ...
    this.processRequest(request)
      ...
      .catch((e: any) =>
        this.log.error(
          `Request ${requestId} failed: ${request.id} to ${request.requestId}: ${e.message}`
        )
      );
  }
  ...
  private async recalibrate(force?: boolean): Promise<boolean> {
    ...
    const isTimeToRecalibrate =
      (nowTimestamp - this.lastRecalibrationTimestamp) / 1000 >
      (this.config.RECALIBRATE || 30);
