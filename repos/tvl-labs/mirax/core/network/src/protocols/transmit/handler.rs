...
use super::{
    protocol::{ProtocolConfig, TransmitCodec},
    types::TransmitMessage,
};

enum InboundSubstreamState {
    /// Waiting for a message from the remote. The idle state for an inbound substream.
    WaitingInput(Framed<Stream, TransmitCodec>),
    /// The substream is being closed.
    Closing(Framed<Stream, TransmitCodec>),
...
enum OutboundSubstreamState {
    ...
    WaitingOutput(Framed<Stream, TransmitCodec>),
    /// Waiting to send a message to the remote.
    PendingSend(Framed<Stream, TransmitCodec>, TransmitMessage),
    /// Waiting to flush the substream so that the data arrives to the remote.
    PendingFlush(Framed<Stream, TransmitCodec>),
    /// An error occurred during processing.
    Poisoned,
}
...
pub struct Handler {
    ...
    pending_send: VecDeque<TransmitMessage>,
...
impl ConnectionHandler for Handler {
    type FromBehaviour = TransmitMessage;
    type ToBehaviour = TransmitMessage;
                ...
                Some(OutboundSubstreamState::PendingSend(mut substream, msg)) => {
                    match Sink::poll_ready(Pin::new(&mut substream), cx) {
                        Poll::Ready(Ok(())) => {
                            ...
                            match Sink::start_send(Pin::new(&mut substream), msg.into()) {
                                Ok(()) => {
                                    self.outbound_substream =
                    ...
                    match substream.poll_next_unpin(cx) {
                        Poll::Ready(Some(Ok(message))) => {
                            ...
                            return Poll::Ready(ConnectionHandlerEvent::NotifyBehaviour(
                                TransmitMessage::from_vec(message.into()),
                            ));
                        }
