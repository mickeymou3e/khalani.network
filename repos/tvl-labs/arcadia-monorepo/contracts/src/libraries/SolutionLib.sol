...
contract SolutionLib {
    ...
    function checkIntentSatisfaction(
        ...
        uint256 minimumFillPercentage
    ) external pure {
        for (uint256 i = 0; i < consumedIntents.length; i++) {
            FillRecord[] memory fillersForCurrInput = getAllFillersFor(i, fillGraph);

            Intent memory inputIntent = consumedIntents[i];
            _checkIntentSatisfaction(
                inputIntent, fillersForCurrInput, createdIntents, createdReceipts, minimumFillPercentage
            );
        }
    }
    ...
    function _checkFilledEAS(
        ...
        if (inputIntent.outcome.mTokens.length != 1) {
            revert SolutionLib__AnySingleExactlyIntentMustHaveExactlyOneOutcomeToken();
        }
        if (inputIntent.outcome.mAmounts.length != 1) {
            revert SolutionLib__AnySingleExactlyIntentMustHaveExactlyOneOutcomeTokenAmount();
        }
    ...
    function _checkFilledPAS(
        ...
        (uint256 initialIntentSrcTokenBalance, uint256 finalIntentSrcTokenBalance) =
            _validateOutputIntent(inputIntent, fillersForCurrInput, createdIntents);

        (uint256 receiptTotal, uint256 percentageExpectedForMTokenReceived) =
            _validateReceipts(inputIntent, fillersForCurrInput, createdReceipts);

        _validateBalancesAndAmounts(
            initialIntentSrcTokenBalance,
            finalIntentSrcTokenBalance,
            receiptTotal,
            percentageExpectedForMTokenReceived,
            minimumFillPercentage
        );
    }

    function _validateOutputIntent(
        Intent memory inputIntent,
        FillRecord[] memory fillersForCurrInput,
        Intent[] memory createdIntents
    ) internal pure returns (uint256 initialIntentSrcTokenBalance, uint256 finalIntentSrcTokenBalance) {
        initialIntentSrcTokenBalance = inputIntent.srcAmount;
        finalIntentSrcTokenBalance = 0;
    ...
    function _validateReceipts(
        Intent memory inputIntent,
        FillRecord[] memory fillersForCurrInput,
        Receipt[] memory createdReceipts
    ) internal pure returns (uint256 receiptTotal, uint256 percentageExpectedForMTokenReceived) {
        receiptTotal = 0;
        address mTokenReceived;
        percentageExpectedForMTokenReceived = 0;
    ...
    function _validateBalancesAndAmounts(
        uint256 initialIntentSrcTokenBalance,
        uint256 finalIntentSrcTokenBalance,
        ...
        uint256 minimumReceiptAmount = (tokensTakenFromInput * minimumFillPercentage) / 1e20;
        ...
        if (receiptTotal < minimumReceiptAmount) {
            revert SolutionLib__MinimumFillAmountNotSatisfied(minimumReceiptAmount, receiptTotal);
        }
    }
