...
contract MTokenManager is AccessManaged {
    // ***************** //
    // **** ERRORS ***** //
    // ***************** //
    ...
    event RefundProcessed(address indexed agent, address indexed user, address indexed mToken, uint256 amount);

    // ***************** //
    // *** VARIABLES *** //
    // ***************** //
    MTokenRegistry private s_tokenRegistry;
    address private s_withdrawalHandler;
    ...
    // retricted to Contracts Manager role
    function setIntentBook(address _intentBook) external restricted {
        s_intentBook = _intentBook;
    }

    function setReceiptManager(address _receiptManager) external restricted {
        s_receiptManager = _receiptManager;
    }

    function setTokenRegistry(address _tokenRegistry) external restricted {
        require(address(s_tokenRegistry) == address(0), "Token registry already set");
        s_tokenRegistry = MTokenRegistry(_tokenRegistry);
    }

    // restricted to Role Admin role
    function setWithdrawalHandler(address _withdrawalHandler) external restricted {
        s_withdrawalHandler = _withdrawalHandler;
    }
    ...
    function withdrawMToken(address from, address mToken, uint256 amount) external payable {
        ...
        SpokeTokenInfo memory spokeInfo = s_tokenRegistry.getSpokeInfoForMToken(mToken);
        address spokeToken = spokeInfo.token;
        uint32 chainId = spokeInfo.chainId;
        WithdrawalHandler(s_withdrawalHandler).withdraw{value: msg.value}(spokeToken, chainId, from, amount);
    }

    function withdrawIntentBalance(bytes32 intentId, address from, address mToken, uint256 amount)
        external
        payable
        restricted
    {
        ...
        if (intentBalance.mToken != mToken) {
            revert MissmatchedMToken();
        }

        uint256 balance = intentBalance.balance;
        if (balance < amount) {
            revert InsufficientMTokens();
        }

        intentBalance.balance = balance - amount;
    ...
    function burnMToken(address operator, address from, uint256 amount, address mToken) external {
        if (msg.sender != mToken) {
            _checkCanCall(_msgSender(), _msgData());
        }
        _validateMToken(mToken);
    ...
    function transferMTokensFromIntent(
        address from,
        bytes32 fromIntentId,
        bytes32 toIntentId,
        address mToken,
        uint256 amount,
        Intent memory spentIntent
    ) external {
        if (msg.sender != s_intentBook) {
            revert CallerNotIntentBook();
        }
        if (spentIntent.author != from) {
            revert AuthorMismatch();
        }

        bytes32 computedIntentId = IntentLib.hashIntent(spentIntent);
        if (computedIntentId != fromIntentId) {
            revert IntentIDMismatch();
        }

        ResourceBalance storage fromIntentBalance = s_intentBalances[fromIntentId];
        if (fromIntentBalance.mToken != mToken) {
            revert MissmatchedMToken();
        }
        if (fromIntentBalance.balance < amount) {
            revert InsufficientIntentBalance(fromIntentBalance.balance, amount);
        }
        fromIntentBalance.balance -= amount;

        ResourceBalance storage toIntentBalance = s_intentBalances[toIntentId];
        toIntentBalance.mToken = mToken;
        toIntentBalance.balance += amount;

        emit MTokenTransferred(mToken, fromIntentId, toIntentId, amount);
    }

    function transferMTokensFromIntent(bytes32 fromIntentId, address to, address mToken, uint256 amount) external {
        if (msg.sender != s_intentBook) {
            revert CallerNotIntentBook();
        }
        ...
        emit MTokenTransferred(mToken, fromIntentId, TypeCasts.addressToBytes32(to), amount);
    }

    function refundUser(address user, address mToken, uint256 amount) external restricted {
        bytes32 userId = TypeCasts.addressToBytes32(user);

        s_userBalances[userId][mToken] += amount;
        s_totalSupply[mToken] += amount;

        emit RefundProcessed(msg.sender, user, mToken, amount);
    }
    ...
    function withdrawMTokensFromReceipt(bytes32 resourceId, address owner, address mToken, uint256 amount) public {
        if (msg.sender != s_receiptManager) {
            revert UnauthorizedCaller();
        }
        _transfer(resourceId, TypeCasts.addressToBytes32(owner), amount, mToken);
    }

    function transferMTokensToReceipt(bytes32 intentId, bytes32 receiptId, address mToken, uint256 amount)
        public
        restricted
    {
    ...
    function getBalanceOfResource(bytes32 resourceId, address mToken) external view returns (uint256) {
        ResourceBalance memory bal = s_intentBalances[resourceId];
        if (bal.mToken == mToken) {
            return bal.balance;
        }
        return 0;
    }
