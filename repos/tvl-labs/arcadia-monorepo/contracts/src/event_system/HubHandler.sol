...
contract HubHandler is Ownable, IEventHandler {
    ...
    error HubHandler__UnrecognizedOriginChain();
    ...
    mapping(uint256 originChainId => bool authorized) public s_authorizedOriginChains;
    ...
    function authorizeOriginChain(uint256 chainId) external onlyOwner {
        s_authorizedOriginChains[chainId] = true;
    }

    function revokeOriginChain(uint256 chainId) external onlyOwner {
        s_authorizedOriginChains[chainId] = false;
    }
    ...
    function handleEvent(XChainEvent calldata ev, uint32 originChain) external override {
        ...
        if (!s_authorizedOriginChains[uint256(originChain)]) {
            revert HubHandler__UnrecognizedOriginChain();
        }
