...
contract MTokenRegistry is AccessManaged {
    ...
    constructor(address _mTokenManager, address _accessManager) AccessManaged(_accessManager) {
    ...
    function createMToken(string memory symbol, string memory name, address spokeAddr, uint32 spokeChainId)
        ...
        returns (address)
    {
        if (s_mTokensPerSpokeToken[spokeChainId][spokeAddr] != address(0)) {
    ...
    function pauseMToken(address mToken) external restricted {
    ...
    function unpauseMToken(address mToken) external restricted {
        MTokenInfo storage mTokenInfo = s_mTokenRegistry[mToken];
        if (mTokenInfo.mTokenAddress == address(0)) {
            revert MTokenRegistry__MTokenNotFound();
        }
    ...
    function destroyMToken(address mToken) external restricted {
        MTokenInfo storage mTokenInfo = s_mTokenRegistry[mToken];
        if (mTokenInfo.mTokenAddress == address(0)) {
            revert MTokenRegistry__MTokenNotFound();
        }
