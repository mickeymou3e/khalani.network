...
import fs from "fs"
import { DebtToken } from "../../typechain-types"
...
export class CoreDeployer {
	...
	async transferContractsOwnerships(contracts) {
		...
		if (!upgradesAdmin || upgradesAdmin == this.hre.ethers.ZeroAddress) {
			throw Error(
	...
	async printDeployerBalance() {
		...
		const cost = prevBalance ? this.hre.ethers.formatUnits(prevBalance - this.deployerBalance) : 0
		console.log(
			`${this.deployerWallet.address} Balance: ${this.hre.ethers.formatUnits(this.deployerBalance)} ${cost ? `(Deployment cost: ${cost})` : ""
			}`
		)
	}
	...
	async updateState(contractName: string, contract: BaseContract, isUpgradeable: boolean) {
		...
		if (isUpgradeable) {
			...
			const implAddress = await getImplementationAddress(provider, await contract.getAddress())
