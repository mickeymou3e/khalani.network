...
class ModelParams:
    def __init__(self):
        ...
        self.T = 5 # weighting for price change in vessel issuance
        self.F = 5 # weighting for momentum change in vessel issuance
...
# return the innate component of market demand for holding SPR tokens.  Could be a function of:
...
def get_new_token_price(data, params, redeemed_amount, momentum):
    ...
    # price = (data.vessel_issuance[-1] - data.token_demand[-1] - ((A + T) * data.token_price[-1]) + ((B + F) * momentum) - redeemed_amount) * factor 
    price = (data.vessel_issuance[-1] - data.innate_token_demand - (A * data.token_price[-1] )  -T + ((B + F) * momentum) - redeemed_amount) * factor 
...
def get_new_vessel_issuance(data, params, token_price, momentum ):
    vessel_issuance = data.vessel_issuance[-1] + params.T*(token_price - 1) + params.F*(momentum)
