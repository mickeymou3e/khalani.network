...
class ModelParams:
    def __init__(self):
        ...
        self.T = 1 # weighting for token price in vessel issuance
        self.F = 0.3 # weighting for momentum in vessel issuance
...
# return the exogenous market demand for holding SPR tokens. Assume constant. Could be a function of:
# - Demand for a safe-haven $1-pegged asset  
# - Traders' demand for short-term liquidity
def get_token_demand():
    return 100.0

# compute price based on setting token supply = vessel demand, and clearing the market
def get_new_token_price(data, params, redeemed_amount, momentum):
    ...
    price = (((data.token_demand  - redeemed_amount) / (data.vessel_issuance[-1])) - (F * momentum)) * factor 
...
def get_new_vessel_issuance(data, params, token_price, momentum ):
    vessel_issuance = data.vessel_issuance[-1] * (params.T*(token_price) + params.F*(momentum))

    if vessel_issuance < 0:
        return 0
    else: 
        return vessel_issuance

def get_new_token_supply(vessel_issuance, redeemed):
    new_supply =  vessel_issuance - redeemed
...
# compute the excess vessel issuance needed to maintain the price at 1.1, according to QTM.
...
for i in range(1, 250):
    ...
    vessel_issuance = get_new_vessel_issuance(data, params, token_price, momentum)
    token_supply = get_new_token_supply(vessel_issuance, redeemed_amount)
    ...
    vessel_issuance = vessel_issuance + excess_issuance
    token_supply = get_new_token_supply(vessel_issuance, 0)
    ...
    print(f'vessel_issuance: {vessel_issuance}')
    ...
    data.vessel_issuance.append(vessel_issuance)
