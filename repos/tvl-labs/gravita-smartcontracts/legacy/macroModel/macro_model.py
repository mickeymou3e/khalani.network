...
def liquidate_vessels(vessels, index, data):
    ...
    price_SPR_previous = data.loc[index - 1, "price_SPR"]
    ...
    airdrop_gain = price_SPR_previous * quantity_SPR_airdrop
...
def SPR_market(index, data):
    quantity_SPR = (100000000 / 3) * (1 - 0.5 ** (index / period))
    ...
    return [price_SPR_current, annualized_earning, MC_SPR_current]
...
for index in range(1, n_sim):
    ...
    if index >= month:
        price_SPR.append(price_SPR_current)
    ...
    new_row = {
        ...
        "MC_SPR": float(MC_SPR_current),
        "price_SPR": float(price_SPR_current),
    }
...
fig = make_subplots(specs=[[{"secondary_y": True}]])
fig.add_trace(
    go.Scatter(x=data.index / 720, y=data["price_SPR"], name="SPR Price"),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data.index / 720, y=data["MC_SPR"], name="SPR Market Cap"),
    secondary_y=True,
)
fig.update_layout(title_text="Dynamics of the Price and Market Cap of SPR")
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="SPR Price", secondary_y=False)
fig.update_yaxes(title_text="SPR Market Cap", secondary_y=True)
fig.show()
...
for index in range(1, n_sim):
    ...
    price_SPR_previous = data2.loc[index - 1, "price_SPR"]
    ...
    # SPR Market
    result_SPR = SPR_market(index, data2)
    price_SPR_current = result_SPR[0]
    annualized_earning = result_SPR[1]
    MC_SPR_current = result_SPR[2]
