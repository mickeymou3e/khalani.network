...
interface IStabilityPool is IDeposit {
	...
	event SPRPaidToDepositor(address indexed _depositor, uint256 _SPR);
	 ...
	 * - _amount is not zero
	 * ---
	 * - Triggers a SPR issuance, based on time passed since the last issuance. The SPR issuance is shared between *all* depositors.
	 * - Sends depositor's accumulated gains (SPR, assets) to depositor
	 */
	 ...
	 * ---
	 * - Triggers a SPR issuance, based on time passed since the last issuance. The SPR issuance is shared between *all* depositors.
	 * - Sends all depositor's accumulated gains (SPR, assets) to depositor
	 ...
	 */
	function getDepositorSPRGain(address _depositor) external view returns (uint256);
