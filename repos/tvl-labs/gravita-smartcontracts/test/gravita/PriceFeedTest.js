...
contract("PriceFeed", async accounts => {
	...
	describe("Custom Aggregators", async () => {
		...
		it("wstETH price via custom aggregator", async () => {
			...
			const wstEth_to_usd_price1 = ethers.formatUnits(wstEth_to_usd_priceBN1.toString(), aggregatorDecimals)

			const expected_wstEth_to_usd_priceBN1 = toBN(WSTETH_TO_STETH)
				.mul(toBN(STETH_TO_USD))
				.div(toBN(dec(1, "ether")))
			const expected_wstEth_to_usd_price = ethers.formatUnits(
			...
			const wstEth_to_usd_price2 = ethers.formatUnits(wstEth_to_usd_priceBN2.toString(), feedDigits)
