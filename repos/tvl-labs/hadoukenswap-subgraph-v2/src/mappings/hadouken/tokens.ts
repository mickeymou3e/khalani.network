...
import { ERC20, Transfer } from '../../types/templates/StablePool/ERC20'
import { log } from '@graphprotocol/graph-ts'
import { PoolToken , TokenBalance }  from '../../types/schema'

function createTokenBalance(tokenAddress: Address, account: Address, timestamp: BigInt): TokenBalance | null {
  let erc = ERC20.bind(tokenAddress)
  let balance = erc.try_balanceOf(account)

  if (balance.reverted) return null

  let tokenBalanceSchema = new TokenBalance(tokenAddress.toHexString() + account.toHexString())
...
export function tokenTransferHandler(event: Transfer): void {
  ...
  let token = PoolToken.load(event.address.toHexString())

  if (token) {
    let tokenBalanceFrom = TokenBalance.load(event.address.toHexString() + sendFrom.toHexString())

    if (!tokenBalanceFrom && sendFrom != ZERO_ADDRESS) {
    ...
    let tokenBalanceTo = TokenBalance.load(event.address.toHexString() + sendTo.toHexString())

    if (!tokenBalanceTo && sendTo != ZERO_ADDRESS) {
