...
import { getPoolAddress, getPoolTokenManager } from './pools'
...
export function handleNewPoolTokens(poolId: Bytes, tokens: Bytes[]): void {
...
export function uptickSwapsForToken(
  tokenAddress: Address,
  event: ethereum.Event
): void {
  let token = getToken(tokenAddress)
...
export function updateTokenBalances(
  ...
  if (swapDirection == SWAP_IN) {
    token.totalBalanceNotional =
      token.totalBalanceNotional.plus(notionalBalance)
    token.totalBalanceUSD = token.totalBalanceUSD.plus(usdBalance)
  } else if (swapDirection == SWAP_OUT) {
    token.totalBalanceNotional =
      token.totalBalanceNotional.minus(notionalBalance)
...
export function getTradePairSnapshot(
  tradePairId: string,
  timestamp: i32
): TradePairSnapshot {
  let dayID = timestamp / 86400
...
export function getBalancerSnapshot(
  vaultId: string,
  timestamp: i32
): BalancerSnapshot {
  let dayID = timestamp / 86400
