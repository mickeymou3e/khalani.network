...
import { MIN_POOL_LIQUIDITY, SWAP_IN, SWAP_OUT } from './helpers/constants'
import { ZERO, ZERO_ADDRESS, ZERO_BD } from '../constants'
import {
  hasVirtualSupply,
...
import { updateAmpFactor } from './StablePool/AmpUpdateStopped'
import { upsertPoolSnapshot } from './poolSnapshot'
...
function handlePoolJoined(event: PoolBalanceChanged): void {
  ...
  join.timestamp = blockTimestamp.toI32()
...
function handlePoolExited(event: PoolBalanceChanged): void {
  ...
  exit.timestamp = blockTimestamp.toI32()
  ...
  for (let i: i32 = 0; i < tokenAddresses.length; i++) {
    ...
    token.totalBalanceNotional =
      token.totalBalanceNotional.minus(tokenAmountOut)
...
export function handleBalanceManage(event: PoolBalanceManaged): void {
  ...
  if (pool == null) {
    log.warning('Pool not found in handleBalanceManage: {}', [
      poolId.toHexString()
    ])
    return
  }
  ...
  let logIndex = event.logIndex
  let transactionHash = event.transaction.hash
  let managementId = transactionHash
    .toHexString()
...
export function handleSwapEvent(event: SwapEvent): void {
  ...
  if (hasVirtualSupply(pool)) {
    if (event.params.tokenIn == pool.address) {
      pool.totalShares = pool.totalShares.minus(
        tokenToDecimal(event.params.amountIn, 18)
      )
    }
    if (event.params.tokenOut == pool.address) {
      pool.totalShares = pool.totalShares.plus(
        tokenToDecimal(event.params.amountOut, 18)
      )
    }
  }
  ...
  tradePairSnapshot.totalSwapVolume =
    tradePair.totalSwapVolume.plus(swapValueUSD)
