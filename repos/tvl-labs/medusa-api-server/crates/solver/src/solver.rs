...
impl Solver {
    pub async fn new(medusa_url: String, key: String) -> Result<Self> {
        ...
        send.send(Message::Text(
            serde_json::to_string(&WsPayload::AddSolver(solver_address)).unwrap(),
        ))
        .await?;
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        send.lock()
            .await
            .send(Message::Text(
                serde_json::to_string(&WsPayload::RequestOpenIntents).unwrap(),
            ))
            .await?;
                ...
                match broadcast_message {
                    WsBroadcastMessage::ExistingOpenIntents(intents) => {
                        ...
                        for intent in intents.iter() {
                            if intent.outcome.fill_structure == FillStructure::PercentageFilled {
                                ...
                                warn!("Intent {} existed before solver started, but it is not a liquidity provision intent. This should never happen.", intent.intent_id());
                            ...
                            let _ = send
                                .send(Message::Text(
                                    serde_json::to_string(&payload).expect(
                                        "Cannot convert signed solution to json rpc string",
                                    ),
                                ))
                                .await;
                    ...
                    WsBroadcastMessage::IntentStatusUpdated(id, status) => match status {
                        IntentState::Open => {
                            ...
                            {
                                warn!("Received message to reopen intent {}, which should never happen.", id);
