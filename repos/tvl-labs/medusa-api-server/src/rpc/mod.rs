...
use medusa_types::{
    Intent, IntentId, RefinementStatus, SignedIntent, SignedIntentId, SignedSolution,
    WsBroadcastMessage,
};
...
use crate::{
    ...
    rpc::{jsonrpc::MedusaRpcImpl, ws::ws_serve},
    storage::StorageService,
};
use medusa_types::IntentHistory;
...
pub trait MedusaRpc {
    ...
    async fn cancel_intent(&self, intent_id: SignedIntentId) -> RpcResult<B256>;
    ...
    async fn request_add_solver(&self, solver_address: SignedAddress) -> RpcResult<()>;
}

pub async fn run_rpc_server<P: Provider<Http<Client>> + 'static + WalletProvider>(
    key_manager: KeyManager,
    rpc_url: SocketAddr,
    ws_url: SocketAddr,
    ...
    ws_serve(ws_url, chain, store, ws_tx, expiration_tx).await;
}
