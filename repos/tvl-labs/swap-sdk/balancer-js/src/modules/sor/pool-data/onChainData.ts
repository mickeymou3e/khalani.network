...
export async function getOnChainBalances(
  ...
  subgraphPoolsOriginal.forEach((pool) => {
    if (!supportedPoolTypes.includes(pool.poolType)) {
      logger.error(`Unknown pool type: ${pool.poolType} ${pool.id}`);
      return;
    }
  ...
  Object.entries(pools).forEach(([poolId, onchainData], index) => {
    try {
      ...
      if (
        ...
        if (!onchainData.amp) {
          logger.error(`Stable Pool Missing Amp: ${poolId}`);
          return;
        } else {
      ...
      if (subgraphPools[index].poolType.includes('Linear')) {
        if (!onchainData.targets) {
          logger.error(`Linear Pool Missing Targets: ${poolId}`);
          return;
        } else {
      ...
      if (
        ...
        if (actualSupply === undefined) {
          logger.error(`ComposableStable missing Actual Supply: ${poolId}`);
          return;
        }
