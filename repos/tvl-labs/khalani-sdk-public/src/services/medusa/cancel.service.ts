...
export interface SignedIntentId {
  intent_id: string
  signature: any
}

export async function cancelIntent(
  signedIntentId: SignedIntentId,
  ...
  try {
    ...
    console.error('Error cancelling intent request:', error)
    if (axios.isAxiosError(error)) {
      const status = error.response?.status
      const statusText = error.response?.statusText
      throw new Error(
        `Error cancelling intent request: ${status} ${statusText}`,
      )
    } else {
      throw new Error(
        'An unexpected error occurred while cancelling intent request',
      )
    }
  }
}
