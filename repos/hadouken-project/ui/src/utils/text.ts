...
export const getTransactionLabel = (transaction: string): string => {
  const first = transaction.substring(0, 6)
  const last = transaction.substring(transaction.length - 4, transaction.length)
  return `${first}...${last}`
}
...
export const convertNumberToStringWithCommas = (
  value: number,
  decimals = 2,
  truncate = false,
): string => {
  const valueAsString = value.toLocaleString(undefined, {
  ...
  if (truncate) {
    ...
    const [wholeValue, decimalValue] = valueAsString.split('.')

    const b = String(parseFloat(`0.${decimalValue}`))

    if (b !== '0') {
      const [, dec] = b.split('.')
      return `${wholeValue}.${dec}`
    } else {
      return `${wholeValue}`
    }
  }
...
export const formatMaxAvailableAmount = (
  maxAmount: BigNumber,
  decimals: number | undefined,
  isFetchingMaxAmount: boolean | undefined,
): string => {
  if (isFetchingMaxAmount || !decimals) return 'Loading...'

  const maxAmountNumber = Number(bigNumberToString(maxAmount, decimals))

  if (maxAmountNumber > 0 && maxAmountNumber < 0.01) return '< 0.01'

  return formatWithMathematicalNotation(Number(maxAmountNumber.toFixed(2)))
}
