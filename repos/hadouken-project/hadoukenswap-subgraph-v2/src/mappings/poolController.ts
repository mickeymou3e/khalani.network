...
import { RecoveryModeStateChanged } from '../types/ComposableStablePoolFactory/ComposableStablePool'
import {
  PausedLocally,
  UnpausedLocally
...
export function handleProtocolFeePercentageCacheUpdated(
...
export function handleRecoveryModeStateChanged(
  event: RecoveryModeStateChanged
): void {
  let poolAddress = event.address
  let poolContract = PoolContract.load(poolAddress.toHexString())
  if (poolContract == null) return
...
export function handleManagementFeePercentageChanged(
  event: ManagementFeePercentageChanged
): void {
  let poolAddress = event.address
  let poolContract = PoolContract.load(poolAddress.toHexString())
  if (poolContract == null) return
...
export function handlePriceRateCacheUpdated(
  event: PriceRateCacheUpdated
): void {
  let poolAddress = event.address
...
export function handleTokenRateCacheUpdated(
