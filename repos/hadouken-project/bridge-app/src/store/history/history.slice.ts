...
import { OperationStatus, TransactionStatus } from '@interfaces/history'
import {
  createSlice,
  PayloadAction,
  createEntityAdapter,
...
import { IHistorySagaState, ITransaction } from './history.types'
...
export const HistorySlice = createSlice({
  ...
  reducers: {
    ...
    closeUnfinishedTransactions: (state) => {
      const ids = state.ids
      ids.forEach((id) => {
        if (state.entities[id].status === TransactionStatus.Pending) {
          state.entities[id].status = TransactionStatus.Fail
        }
