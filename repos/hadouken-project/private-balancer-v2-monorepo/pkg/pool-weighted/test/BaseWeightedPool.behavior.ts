...
export function itBehavesAsWeightedPool(numberOfTokens: number, poolType: WeightedPoolType): void {
  ...
  describe('creation', () => {
    ...
    context('when the creation fails', () => {
      it('reverts if the number of tokens and weights do not match', async () => {
        ...
        await expect(deployPool({ weights: badWeights })).to.be.reverted;
      });

      it('reverts if there are repeated tokens', async () => {
        const badTokens = new TokenList(Array(numberOfTokens).fill(tokens.first));

        await expect(deployPool({ tokens: badTokens, fromFactory: true })).to.be.reverted;
      });

      it('reverts if the swap fee is too high', async () => {
        const badSwapFeePercentage = fp(0.1).add(1);

        await expect(deployPool({ swapFeePercentage: badSwapFeePercentage })).to.be.reverted;
      });
  ...
  describe('onJoinPool', () => {
    function itJoins() {
      it('fails if caller is not the vault', async () => {
        await expect(pool.instance.connect(lp).onJoinPool(pool.poolId, lp.address, other.address, [0], 0, 0, '0x')).to
          .be.reverted;
      });

      it('fails if no user data', async () => {
        await expect(pool.join({ data: '0x', from: lp })).to.be.reverted;
      });

      it('fails if wrong user data', async () => {
        const wrongUserData = ethers.utils.defaultAbiCoder.encode(['address'], [lp.address]);

        await expect(pool.join({ data: wrongUserData, from: lp })).to.be.reverted;
      });
      ...
      context('initialization', () => {
        ...
        it('fails if already initialized', async () => {
          ...
          await expect(pool.init({ initialBalances, from: lp })).to.be.reverted;
        });
      ...
      context('join exact tokens in for BPT out', () => {
        it('fails if not initialized', async () => {
          await expect(pool.joinGivenIn({ recipient, amountsIn: initialBalances, from: lp })).to.be.reverted;
        });
        ...
        context('once initialized', () => {
          ...
          it('fails if not enough BPT', async () => {
            ...
            await expect(pool.joinGivenIn({ amountsIn, minimumBptOut, from: lp })).to.be.reverted;
          });

          it('reverts if paused', async () => {
            await pool.pause();

            await expect(pool.joinGivenIn({ amountsIn })).to.be.reverted;
          });
        });
      });
      ...
      context('join token in for exact BPT out', () => {
        ...
        it('fails if not initialized', async () => {
          await expect(pool.joinGivenOut({ bptOut, token, from: lp })).to.be.reverted;
        });
        ...
        context('once initialized', () => {
          ...
          it('reverts if paused', async () => {
            ...
            await expect(pool.joinGivenOut({ bptOut, token })).to.be.reverted;
          });
        });
      });
      ...
      context('join all tokens in for exact BPT out', () => {
        it('fails if not initialized', async () => {
          await expect(pool.joinAllGivenOut({ bptOut: fp(2), from: lp })).to.be.reverted;
        });
        ...
        context('once initialized', () => {
          ...
          it('reverts if paused', async () => {
            ...
            await expect(pool.joinAllGivenOut({ bptOut: fp(2) })).to.be.reverted;
          });
        });
      });
    }
  ...
  describe('onExitPool', () => {
    ...
    function itExits() {
      ...
      it('fails if no user data', async () => {
        await expect(pool.exit({ data: '0x' })).to.be.reverted;
      });

      it('fails if wrong user data', async () => {
        const wrongUserData = ethers.utils.defaultAbiCoder.encode(['address'], [lp.address]);

        await expect(pool.exit({ data: wrongUserData })).to.be.reverted;
      });
      ...
      context('exit exact BPT in for one token out', () => {
        ...
        it('fails if invariant decreases more than max allowed', async () => {
          ...
          await expect(pool.singleExitGivenIn({ bptIn, token })).to.be.reverted;
        });
      ...
      context('exit exact BPT in for all tokens out', () => {
        ...
        it('reverts if paused', async () => {
          ...
          const bptIn = previousBptBalance.div(2);
          await expect(pool.multiExitGivenIn({ from: lp, bptIn })).to.be.reverted;
        });
      });
      ...
      context('exit BPT in for exact tokens out', () => {
        ...
        it('fails if more BTP needed', async () => {
          ...
          await expect(pool.exitGivenOut({ from: lp, amountsOut, maximumBptIn })).to.be.reverted;
        });
        ...
        it('reverts if paused', async () => {
          ...
          const amountsOut = initialBalances;
          await expect(pool.exitGivenOut({ from: lp, amountsOut })).to.be.reverted;
        });
      });
    }
    ...
    function itSwaps() {
      context('given in', () => {
        ...
        it('reverts if token in exceeds max in ratio', async () => {
          ...
          await expect(pool.swapGivenIn({ in: 1, out: 0, amount, from: lp })).to.be.reverted;
        });

        it('reverts if token in is not in the pool', async () => {
          await expect(pool.swapGivenIn({ in: allTokens.GRT, out: 0, amount: 1, from: lp })).to.be.reverted;
        });

        it('reverts if token out is not in the pool', async () => {
          await expect(pool.swapGivenIn({ in: 1, out: allTokens.GRT, amount: 1, from: lp })).to.be.reverted;
        });

        it('reverts if paused', async () => {
          await pool.pause();

          await expect(pool.swapGivenIn({ in: 1, out: 0, amount: 1, from: lp })).to.be.reverted;
        });
      });
      ...
      context('given out', () => {
        ...
        it('reverts if token in exceeds max out ratio', async () => {
          ...
          await expect(pool.swapGivenOut({ in: 1, out: 0, amount, from: lp })).to.be.reverted;
        });

        it('reverts if token in is not in the pool when given out', async () => {
          await expect(pool.swapGivenOut({ in: allTokens.GRT, out: 0, amount: 1, from: lp })).to.be.reverted;
        });

        it('reverts if token out is not in the pool', async () => {
          await expect(pool.swapGivenOut({ in: 1, out: allTokens.GRT, amount: 1, from: lp })).to.be.reverted;
        });

        it('reverts if paused', async () => {
          await pool.pause();

          await expect(pool.swapGivenOut({ in: 1, out: 0, amount: 1, from: lp })).to.be.reverted;
        });
      });
    }
