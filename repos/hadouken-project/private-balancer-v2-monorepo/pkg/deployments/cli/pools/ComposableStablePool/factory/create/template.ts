...
import { ComposableStablePool__factory } from '@hadouken-project/typechain';
...
export const ComposableStablePoolTemplateCreateCli: Cli = async (cliProps) => {
  ...
  try {
    ...
    const { template } = await prompts(
      {
        ...
        choices: fileNames
          .filter((fileName) => fileName.includes(cliProps.environment.network))
          .map((fileName) => ({
    ...
    const createParameters: ComposableStablePoolFactoryCreateParameters = {
      ...
      environment: cliProps.environment,
    };

    const output = await create(createParameters);

    if (output) {
      save<Output<Omit<ComposableStablePoolFactoryCreateParameters, 'environment'>, string>>(
        output,
        input.symbol,
        __dirname,
      ...
      const pool = ComposableStablePool__factory.connect(
        output?.data['ComposableStablePoolFactory'].create.output,
        deployer
      );
      ...
      await initJoin(cliProps.environment, vaultAddress, pool.address, input.initialLiquidity);
    }
