...
describe('BALTokenHolder', function () {
  ...
  describe('withdrawFunds', () => {
    ...
    context('when the caller is not authorized', () => {
      it('reverts', async () => {
        await expect(holder.connect(other).withdrawFunds(other.address, 100)).to.be.revertedWith('BAL#401');
      });
    });
  });
  ...
  describe('sweepTokens', () => {
    ...
    context('when the caller is not authorized', () => {
      it('reverts', async () => {
        await expect(holder.connect(other).sweepTokens(DAI.address, other.address, 100)).to.be.revertedWith('BAL#401');
      });
    });
  });
});
