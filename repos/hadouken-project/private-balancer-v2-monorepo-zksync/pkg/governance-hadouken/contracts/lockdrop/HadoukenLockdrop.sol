...
contract HadoukenLockdrop is Ownable(msg.sender) {
    ...
    uint256 public constant PHASE_ONE_DURATION = 20 minutes;
    uint256 public PHASE_ONE_START_TIME;
    uint256 public PHASE_ONE_END_TIME;

    uint256 public constant PHASE_TWO_DURATION = 20 minutes;
    ...
    event FinalizePhaseOne(uint256 phaseStartTime, address husdToken, uint256 husdPrice, address triCryptoToken, uint256 triCryptoPrice);
    event FinalizePhaseTwo(uint256 phaseStartTime, uint256 hdkAmount, uint256 priceTokenAmount, uint256 totalBpt);


    constructor(uint256 phaseOneStartTime, IERC20 _husdToken, IERC20 _triCryptoToken, IERC20 _hdkToken, IERC20 _priceToken, IVault _vault) {
     ...
     */
    function lock(address token, uint256 amount, LockLength lockLength) external isPhaseOne {
    ...
    function finalizePhaseOne(
        ...
        emit FinalizePhaseOne(phaseTwoStartTime, address(husdToken), _husdPrice, address(triCryptoToken), _triCryptoPrice);
    }
    ...
    function depositPhaseTwo(uint256 hdkTokenAmount, uint256 _priceTokenAmount) external payable isPhaseTwo {
        uint256 priceTokenAmount = msg.value;
    ...
    function finalizePhaseTwo(uint256 phaseThreeStartTime, IBasePool _basePool, uint256 hdkBonus) external phaseTwoHasEnded onlyOwner {
        ...
        hdkAmount = totalDepositedHDK + hdkBonus;
        priceTokenJoined = address(this).balance;

        hdkToken.transferFrom(msg.sender, address(this), hdkBonus + DUST);
        ...
        vault.joinPool{value: priceTokenJoined}(id, address(this), address(this), request);
    ...
    function getUserTotalLP(address user) public view returns (uint256) {
        uint256 rate = (hdkAmount * PRECISION) / priceTokenJoined;
        uint256 userTotalHDK = depositedHDK[user] + ((depositedpriceToken[user] * rate) / PRECISION);
        uint256 poolTotalHDK = hdkAmount + ((priceTokenJoined * rate) / PRECISION);
