...
contract AaveLinearPoolRebalancer is LinearPoolRebalancer, Ownable {
    bool private _isInitializedLinearPool;

    // These Rebalancers can only be deployed from a factory to work around a circular dependency: the Pool must know
    // the address of the Rebalancer in order to register it, and the Rebalancer must know the address of the Pool
    // during construction.
    constructor(IVault vault, IBalancerQueries queries) LinearPoolRebalancer(vault, queries) {
        // solhint-disable-previous-line no-empty-blocks
        _isInitializedLinearPool = false;
    }
    ...
    function initialize(ILinearPool pool) external override onlyOwner {
        require(_isInitializedLinearPool == false, "Pool is already initialized");
