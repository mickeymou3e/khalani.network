...
abstract contract ComposableStablePoolStorage is BasePool, IRateProviderPool {
    ...
    constructor(StorageParams memory params) {
        ...
        _token3 = totalTokens > 3 ? params.registeredTokens[3] : IERC20(address(0));
        _token4 = totalTokens > 4 ? params.registeredTokens[4] : IERC20(address(0));
        _token5 = totalTokens > 5 ? params.registeredTokens[5] : IERC20(address(0));
        ...
        for (uint256 i = 0; i < params.registeredTokens.length; ++i) {
            if (i < bptIndex) {
                ...
                if (params.exemptFromYieldProtocolFeeFlags[i]) {
                    _require(rateProviders[i] != IRateProvider(address(0)), Errors.TOKEN_DOES_NOT_HAVE_RATE_PROVIDER);
        ...
        _rateProvider3 = (rateProviders.length > 3) ? rateProviders[3] : IRateProvider(address(0));
        _rateProvider4 = (rateProviders.length > 4) ? rateProviders[4] : IRateProvider(address(0));
        _rateProvider5 = (rateProviders.length > 5) ? rateProviders[5] : IRateProvider(address(0));
