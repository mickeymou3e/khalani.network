...
abstract contract WeightedPoolProtocolFees is BaseWeightedPool, ProtocolFeeCache, IRateProviderPool {
    ...
    constructor(uint256 numTokens, IRateProvider[] memory rateProviders) {
        ...
        _rateProvider2 = numTokens > 2 ? rateProviders[2] : IRateProvider(address(0));
        _rateProvider3 = numTokens > 3 ? rateProviders[3] : IRateProvider(address(0));
        _rateProvider4 = numTokens > 4 ? rateProviders[4] : IRateProvider(address(0));
        _rateProvider5 = numTokens > 5 ? rateProviders[5] : IRateProvider(address(0));
        _rateProvider6 = numTokens > 6 ? rateProviders[6] : IRateProvider(address(0));
        _rateProvider7 = numTokens > 7 ? rateProviders[7] : IRateProvider(address(0));
    }
    ...
    function _getRateFactor(uint256 normalizedWeight, IRateProvider provider) internal view returns (uint256) {
        return provider == IRateProvider(address(0)) ? FixedPoint.ONE : provider.getRate().powDown(normalizedWeight);
    }
