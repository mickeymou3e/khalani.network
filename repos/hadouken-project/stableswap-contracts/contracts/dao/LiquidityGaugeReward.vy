...
def __init__(lp_addr: address, _minter: address, _reward_contract: address, _rewarded_token: address, _admin: address):
    ...
    self.inflation_rate = HDK20(crv_addr).rate()
    self.future_epoch_time = HDK20(crv_addr).future_epoch_time_write()
...
def _checkpoint_rewards(addr: address, claim_rewards: bool):
    ...
    if claim_rewards:
        ...
        HadoukenRewards(self.reward_contract).getReward()
...
def _checkpoint(addr: address, claim_rewards: bool):
    ...
    if prev_future_epoch >= _period_time:
        self.future_epoch_time = HDK20(_token).future_epoch_time_write()
        new_rate = HDK20(_token).rate()
...
def claimable_reward(addr: address) -> uint256:
    ...
    """
    d_reward: uint256 = HadoukenRewards(self.reward_contract).earned(self)
...
def deposit(_value: uint256, addr: address = msg.sender):
    ...
    if _value != 0:
        ...
        HadoukenRewards(self.reward_contract).stake(_value)
...
def withdraw(_value: uint256, claim_rewards: bool = True):
    ...
    if _value > 0:
        HadoukenRewards(self.reward_contract).withdraw(_value)
...
def toggle_external_rewards_claim(val: bool):
    ...
    @notice Switch claiming rewards on/off. 
            This is to prevent a malicious rewards contract from preventing HDK claiming
    """ 
    assert msg.sender == self.admin
