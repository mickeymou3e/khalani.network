# @version 0.2.7
"""
@title Hadouken StableSwap Proxy
@author Hadouken Finance
@license MIT
"""
...
ADDRESS_PROVIDER: constant(address) = 0x352295403b85F2A59882974B0f00c09F9e70C7d6
...
def withdraw_admin_fees(_pool: address):
    ...
    """
    Hadouken(_pool).withdraw_admin_fees()
...
def withdraw_many(_pools: address[20]):
    ...
    for pool in _pools:
        if pool == ZERO_ADDRESS:
            break
        Hadouken(pool).withdraw_admin_fees()
...
def commit_transfer_ownership(_pool: address, new_owner: address):
    ...
    """
    assert msg.sender == self.ownership_admin, "Access denied"
    Hadouken(_pool).commit_transfer_ownership(new_owner)
...
def apply_transfer_ownership(_pool: address):
    ...
    """
    Hadouken(_pool).apply_transfer_ownership()
...
def accept_transfer_ownership(_pool: address):
    ...
    @param _pool Pool address
    """
    Hadouken(_pool).accept_transfer_ownership()
...
def revert_transfer_ownership(_pool: address):
    ...
    Hadouken(_pool).revert_transfer_ownership()
...
def commit_new_parameters(_pool: address,
    ...
    Hadouken(_pool).commit_new_parameters(amplification, new_fee, new_admin_fee)  # dev: if implemented by the pool
...
def apply_new_parameters(_pool: address):
    ...
    Hadouken(_pool).apply_new_parameters()  # dev: if implemented by the pool
...
def revert_new_parameters(_pool: address):
    ...
    Hadouken(_pool).revert_new_parameters()  # dev: if implemented by the pool
...
def commit_new_fee(_pool: address, new_fee: uint256, new_admin_fee: uint256):
    ...
    """
    assert msg.sender == self.parameter_admin, "Access denied"
    Hadouken(_pool).commit_new_fee(new_fee, new_admin_fee)
...
def ramp_A(_pool: address, _future_A: uint256, _future_time: uint256):
    ...
    """
    assert msg.sender == self.parameter_admin, "Access denied"
    Hadouken(_pool).ramp_A(_future_A, _future_time)
...
def set_aave_referral(_pool: address, referral_code: uint256):
    ...
    """
    assert msg.sender == self.ownership_admin, "Access denied"
    Hadouken(_pool).set_aave_referral(referral_code)  # dev: if implemented by the pool
...
def donate_admin_fees(_pool: address):
    ...
    Hadouken(_pool).donate_admin_fees()  # dev: if implemented by the pool