export const getErrorMessageFromLendingContracts = (value: number): string => {
  switch (value) {
    case 1:
    ...
    case 6:
      return 'Transfer cannot be executed due to a low health factor.'
    case 7:
    ...
    case 92:
      return 'Invalid borrow cap'
    case 93:
      return 'Invalid deposit cap'
    default:
      return 'Unknown error occurred'
  }
}

export const getLendingError = (error: string): string | null => {
  const revertMessage = 'reverted with reason string "'
  const revertMessageTwo = '"execution reverted: '

  const errorMessage = error.split(revertMessage)
  const errorMessageTwo = error.split(revertMessageTwo)

  if (errorMessage.length > 1) {
    const errorCode = Number(errorMessage[1].split(`"`)[0])

    return getErrorMessageFromLendingContracts(errorCode)
  } else if (errorMessageTwo.length > 1) {
    const errorCode = Number(errorMessageTwo[1].split(`"`)[0])
    return getErrorMessageFromLendingContracts(errorCode)
  }
