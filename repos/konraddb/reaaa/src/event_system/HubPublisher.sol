...
contract HubPublisher is Ownable {
    ...
    function publishEvent(bytes calldata eventData, bytes32 eventType, uint32 chainId) external payable onlyProducer {
    ...
    function revokeProducerAccessFull(address producer) private {
    ...
    function revokeProduceAccessPartial(address producer, bytes32 eventStructTypeHash) private {
        bytes32 eventType = keccak256(abi.encode(producer, eventStructTypeHash));
