...
import {AbstractAIPEventHandler} from "./AbstractAIPEventHandler.sol";

contract AIPEventHandler is AbstractAIPEventHandler {
    MTokenManager public immutable mTokenManager;
    AssetReserves public assetReserves;

    bool public isArcadia;
    ...
    constructor(address _mTokenManager, address _assetReserves, bool _isArcadia, address _owner)
        AbstractAIPEventHandler(_owner)
    {
        mTokenManager = MTokenManager(_mTokenManager);
        assetReserves = AssetReserves(_assetReserves);
        isArcadia = _isArcadia;
    }

    function setAssetReserves(address _assetReserves) external {
        require(_assetReserves != address(0), "Invalid AssetReserves address");
        assetReserves = AssetReserves(_assetReserves);
    }

    function registerEventVerifier(address eventVerifier) external override {
    ...
    function unregisterEventVerifier(address eventVerifier) external override {
