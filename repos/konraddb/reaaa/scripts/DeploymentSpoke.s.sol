...
contract DeploymentSpoke is Script {
    ...
    function run() external {
        ...
        address gasOracle = address(new GasAmountOracle());
        address arcadiaHubChainEventVerifier = getHubEventVerifier();
        GasAmountOracle(gasOracle).setGasAmount(uint32(spokeChainId), uint32(hubChainId), 500000);
        address spokeHandler = _deploySpokeEventHandler();
        address spokePublisher = _deploySpokePublisher(uint32(hubChainId));
        address spokeEventVerifier = _deployEventVerifier(spokeMailbox, spokeHandler);
        address spokeEventProver =
            _deployEventProver(spokeMailbox, arcadiaHubChainEventVerifier, uint32(hubChainId), igp, gasOracle);
        EventProver(spokeEventProver).addEventPublisher(spokePublisher);

        address assetReserves = _deployAssetReserves(spokeDeployer, spokePublisher, spokeHandler, permit2Address);
        _setupDeps(
            spokeHandler, spokeEventProver, spokeEventVerifier, spokePublisher, assetReserves, uint32(hubChainId)
        );
    }
    ...
    function _deployEventProver(
        address spokeMailbox,
        address hubChainEventVerifier,
        uint32 hubChainId,
        address igp,
        address gasOracle
    ) internal returns (address spokeEventProver) {
        spokeEventProver = address(new EventProver(spokeMailbox, hubChainEventVerifier, hubChainId, igp, gasOracle));
    }
