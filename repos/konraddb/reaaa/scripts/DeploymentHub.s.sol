...
contract DeploymentHub is Script {
    ...
    function run() external {
        ...
        address spokeChainEventVerifier = vm.envAddress("SPOKE_CHAIN_EVENT_VERIFIER");
        ...
        _setupDeps(hubDeployer, eventProver, hubPublisher, uint32(spokeChainId), hubHandler, hubEventVerifier);
        vm.stopBroadcast();
    ...
    function _setupDeps(
        address admin,
        address eventProver,
        address hubPublisher,
        uint32 spokeChainId,
        address hubHandler,
        address hubEventVerifier
    ) internal {
