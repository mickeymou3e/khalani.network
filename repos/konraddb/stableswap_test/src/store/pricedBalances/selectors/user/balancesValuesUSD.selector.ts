...
import { balancesValuesUSDSelectors } from '../balancesValuesUSD.selector'
...
const selectUserPoolValuesUSD = createSelector(
  ...
  (selectPoolValuesUSDByAddress, userAddress) => {
...
const selectUserTotalPoolValueUSD = createSelector(
  ...
  (selectUserPoolValuesUSD) => {
    return (poolId: IPool['id']) => {
      if (poolId) {
        const valuesUSD = selectUserPoolValuesUSD(poolId)
        const totalValueUSD =
          valuesUSD &&
          Object.keys(valuesUSD).reduce(
            (totalValueUSD, token) =>
              valuesUSD[token]
                ? totalValueUSD.add(valuesUSD[token] as BigDecimal)
                : totalValueUSD,
            BigDecimal.from(0),
          )
