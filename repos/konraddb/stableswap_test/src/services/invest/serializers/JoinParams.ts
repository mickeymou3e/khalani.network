...
import { PoolType, IPool } from '@interfaces/pool'
...
export default class JoinParams {
  private readonly dataEncodeFn: (data: any) => string
  ...
  public serialize(
    ...
    if (this.pool.poolType === PoolType.ComposableStable) {
      const lpIndex = tokensIn.findIndex(
        (token) => token.address === this.pool.address,
      )
    ...
    const joinPoolRequest: JoinPoolRequest = {
      assets: tokensIn.map(({ address }) => address),
