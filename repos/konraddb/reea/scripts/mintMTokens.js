...
async function main() {
  ...
  const mintToAddress = deployer.address;
    ...
    for (const [tokenSymbol, tokenAddress] of Object.entries(tokenList)) {
      ...
      console.log(
        `Minting ${ethers.formatEther(
          mintAmount
        )} ${tokenSymbol}.${networkKey} to ${mintToAddress} using MTokenManager...`
      );
      ...
      console.log(
        `${ethers.formatEther(
          mintAmount
        )} ${tokenSymbol} minted to ${mintToAddress} on ${networkKey} using MTokenManager.`
      );
    }
  }
