...
contract MTokenManager {
    ...
    error InsufficientIntentBalance();
    ...
    // *** VARIABLES *** //
    // ***************** //
    AuthorizationManager private immutable i_authManager;
    MTokenRegistry private i_tokenRegistry;
    ...
    constructor(address _authManager, address _aipEventPublisher) {
        i_authManager = AuthorizationManager(_authManager);
    ...
    function setIntentBook(address _intentBook) external {
        // Add proper access control
        s_intentBook = _intentBook;
    }

    function setReceiptManager(address _receiptManager) external {
        // Add proper access control
        s_receiptManager = _receiptManager;
    }

    function setTokenRegistry(address _tokenRegistry) external {
        require(address(i_tokenRegistry) == address(0), "Token registry already set");
        i_tokenRegistry = MTokenRegistry(_tokenRegistry);
    }
    ...
    function mintMToken(address to, address mToken, uint256 amount) external {
        if (msg.sender != mToken) {
            i_authManager.checkAuthorizedMinter(msg.sender);
        }
    ...
    function mTokenTransferFrom(address spender, address from, address to, uint256 amount, address mToken) external onlyMToken(mToken) {
        _validateMToken(mToken);
    ...
    function mTokenApprove(address owner, address spender, uint256 amount, address mToken) external onlyMToken(mToken) {
        bytes32 ownerId = TypeCasts.addressToBytes32(owner);
        bytes32 spenderId = TypeCasts.addressToBytes32(spender);
    ...
    function transferMTokensToIntent(
        ...
        bytes32 computedIntentId = IntentLib.hashIntent(signedIntent.intent, s_intentBook);
    ...
    function transferMTokensToReceipt(
        bytes32 intentId,
        bytes32 receiptId,
        address mToken,
        uint256 amount
    ) public {
        i_tokenRegistry.checkValidMToken(mToken);
        i_authManager.checkAuthorizedMinter(msg.sender);

        ResourceBalance storage intentBalance = s_intentBalances[intentId];
        if (intentBalance.mToken != mToken || intentBalance.balance < amount) {
            revert InsufficientIntentBalance();
        }
    ...
    function _validateMToken(address mToken) private view {
        MTokenInfo memory mTokenInfo = i_tokenRegistry.getMTokenInfo(mToken);
