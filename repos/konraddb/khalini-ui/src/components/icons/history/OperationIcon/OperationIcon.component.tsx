...
const OperationIcon: React.FC<IOperationIconProps> = ({ status }) => {
  ...
  switch (status) {
    case OperationStatus.Pending:
      return (
        <CircularProgress
          style={{ color: theme.palette.common.white, width: 24, height: 24 }}
        />
      )
    case OperationStatus.Success:
      return (
        <CheckCircleIcon style={{ color: theme.palette.quaternary.main }} />
      )
    ...
    case OperationStatus.Waiting:
      return (
        <PauseCircleFilledIcon style={{ color: theme.palette.tertiary.main }} />
      )
    default:
