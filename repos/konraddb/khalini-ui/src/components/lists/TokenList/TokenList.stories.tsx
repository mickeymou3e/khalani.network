...
const Template: Story<ComponentProps<typeof TokenList>> = () => {
  ...
  const handlePopularTokenList = (id: string) => {
    ...
    const filteredTokens = popularTokensList?.filter((token) => token.id !== id)
  ...
  const handleSelectedTokenList = (id: string) => {
    ...
    const filteredToken = selectedTokenList?.filter((token) => token.id !== id)
