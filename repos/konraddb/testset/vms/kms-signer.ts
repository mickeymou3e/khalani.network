...
export class KmsEvmSigner implements EvmSigner {
  constructor(public web3: Web3, public address: string) {}
  ...
  static async create(
    ...
    const provider = new KmsProvider(rpcUrl, {
      keyIds: [kmsKeyId],
      region: awsRegion,
    });
    ...
    engine.on("error", (e?: any) =>
      log.error(`An error occurred in HTTP provider of ${chainID}: ${e}`)
    );
    const addresses = await provider.getAccounts();
    const address = addresses[0];
    console.log(
      `Creating Web3 KMS provider with address ${address} for chain ${chainID}`
    );
  ...
  async signTransaction(
    ...
    const rlpEncodedTransaction = await this.web3.eth.signTransaction(
      transactionConfig
    );
