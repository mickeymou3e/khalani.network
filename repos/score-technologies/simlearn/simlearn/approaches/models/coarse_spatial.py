...
class CoarseSpatial(nn.Module):
    def __init__(self, grid_size=(25, 40), initial_sigma_player=5.0, 
        ...
        super(CoarseSpatial, self).__init__()
        self.grid_size = grid_size
        self.slow_ball_threshold = slow_ball_threshold  # Speed threshold for "slowly moving" ball
        self.defensive_cone_angle = math.radians(defensive_cone_angle)  # Convert to radians
        ...
        self.goal_importance = (self.goal_importance / self.goal_importance.max()).float()  # Normalize to [0,1]
        self.register_buffer('goal_importance_map', self.goal_importance)

        # Shallow conv layers
        self.conv_layers = nn.Sequential(
            nn.Conv2d(8, 16, kernel_size=3, padding=1),  # Changed from 6 to 8 channels
            nn.ReLU(),
            nn.MaxPool2d(2),
    ...
    def _calculate_defensive_pressure(self, ball_pos, team1_positions, team2_positions):
        """Calculate defensive pressure as a simplified metric."""
        batch_size = ball_pos.shape[0]
        device = ball_pos.device
        ...
        # Calculate distances from ball to goals
        ball_to_goal1 = torch.norm(ball_pos - goal1_pos, dim=1)
        ball_to_goal2 = torch.norm(ball_pos - goal2_pos, dim=1)
        
        # Calculate minimum distances from each team to the ball
        team1_distances = torch.norm(team1_positions - ball_pos.unsqueeze(1), dim=2)
        team2_distances = torch.norm(team2_positions - ball_pos.unsqueeze(1), dim=2)
        min_team1_dist = torch.min(team1_distances, dim=1)[0]
        min_team2_dist = torch.min(team2_distances, dim=1)[0]
        
        # Calculate defensive pressure using torch.where
        team1_pressure = (min_team1_dist < 10.0) & (ball_to_goal2 < 30.0)
        team2_pressure = (min_team2_dist < 10.0) & (ball_to_goal1 < 30.0)
        
        # Initialize pressure tensor
        pressure = torch.zeros(batch_size, device=device, dtype=torch.float32)
        
        # Use torch.where to set values
    ...
    def forward(self, x):
        ...
        spatial_features = torch.stack([
            ...
            team2_possession,
            possession_map,
            defensive_pressure.unsqueeze(1).unsqueeze(2).expand(-1, *self.grid_size),  # Add defensive pressure as a new channel
            torch.zeros_like(team1_influence)  # Keep 8 channels by adding a zero channel
        ], dim=1)
